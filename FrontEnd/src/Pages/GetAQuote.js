...
function GetAQuote() {
  ...
  const makersServices = useSelector((state) => state.makersServices.services);
      ...
      if (makersList && makersServices && selectedMethod) {
        ...
        const filteredMakersList = makersList.filter((maker) => {
            for (let i = 0; i < filteredServices.length; i++) {
              if (maker.Manufacturer_ID === filteredServices[i].Manufacturer_ID)
                return maker;
            }
          });
        console.log(filteredMakersList, "filteredMakersList")
        if (filteredMakersList && filteredMakersList.length > 0) {
          for (let i = 0; i < filteredMakersList.length; i++) {
            if (filteredMakersList[i].Logo !== null) {
              const imageData = JSON.parse(filteredMakersList[i].Logo);
