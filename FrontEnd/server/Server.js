...
// var ipAddress = "192.168.88.216";
// var hostAddress = "http://192.168.88.216:3000";
...
app.use(function (req, res, next) {
  ...
  res.setHeader("Access-Control-Allow-Origin", "http://192.168.88.216:3000");
      ...
      if (index == -1) {
        bcrypt.hash(password, saltRound, (err, hash) => {
          db.query(
            ...
            [firstName, lastName, hash, email, phoneNumber, address, 0],
            (err, result) => {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        if (result.length === 0) {
          ...
          db.query("UPDATE manufacturer SET Password =? WHERE Email = ?", [
            hash,
            email,
          ]);
        } else {
...
app.get("/validation", (req, res) => {
  try {
    ...
    db.query("SELECT * FROM order_specification", (err, orderSpecification) => {
      if (err) {
        return console.log(err);
      }
          ...
          bcrypt.compare(
            ...
            (error, response) => {
              if (response) {
                ...
                bcrypt.hash(new_password, saltRound, (err, hash) => {
                  db.query(
              ...
              if (response) {
                ...
                bcrypt.hash(new_password, saltRound, (err, hash) => {
                  db.query(
                    ...
                    [hash, id],
                    (err, result) => {
                      console.log("updated", new_password);
                      res.send({
                        msg: "Password Changed Sucessfully",
                      });
                    }
      ...
      if (err) {
        ...
        await new Promise((resolve) => {
          hubServices.forEach((hubService) => {
            const serviceID = hubService.selectedFabrication.Service_ID;
