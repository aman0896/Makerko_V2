...
router.post('/maker-edit', (req, res) => {
	...
	upload(req, res, async (err) => {
		...
		if (err) {
			...
			const sqlQuery =
				'UPDATE manufacturer SET Company_Name=?, Logo=?, Contact_Person=?,Email=?,Phone_Number=?, Address = ?,Company_Type=?, Brief_Description = ?, Additional_Details = ? WHERE Manufacturer_ID = ?';
			const data = [
